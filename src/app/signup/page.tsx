...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (formData.get('password') !== formData.get('repassword')) {
      toast.error(t('error-match-password'), {
        richColors: true
      });
      return;
    }
    ...
    try {
      ...
      toast.error(t('error', { error: (error as Error).message }), {
        important: true,
        richColors: true
      });
    } finally {
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-100 px-2">
      <div className="w-full max-w-md rounded-lg bg-white p-4 shadow-md md:p-8">
        ...
        <p className="text-center text-sm text-gray-600">
          ...
          <Link className="font-bold underline hover:text-black" href="/signin">
            {t('signin')}
          </Link>
        </p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <Input id="flow" name="flow" type="hidden" value="signUp" />
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              name="repassword"
              type="password"
              placeholder={t('re-password-placeholder')}
              required
              minLength={3}
              autoComplete="new-password"
            />
